...
class DictDiff(object):
  ...
  def __unicode__(self):
    ...
    if self._left_missing_keys:
      parts.append('Keys missing from left but available in right: %s' % self._left_missing_keys)
    if self._right_missing_keys:
      parts.append('Keys available in left but missing from right: %s' % self._right_missing_keys)
